...
# 0. Set up a Python "venv" in this directory:
#   python3 -m venv venv
#   source ./venv/bin/activate
# 1. create a file in the same directory as this script, named "config.py"
# 2. copy paste this line:
#   path_project = "/local/path/to/this/repo/joi.github.io"
#   (which should be the path to the directory one level up from where this file here is.)
# 3. Install the required Python packages using PIP & the requirements.txt file
...
# This imports our local config file, "config.py". Access vars like so: config.var
import config
import shutil
import re
import copy
...
# manually set in config.py, which is NOT checked into GIT.
path_project = config.path_project
# automatically detects the User home directory path from the OS/ENV
...
# Paparika Timestamp Offset: 978307200 -> 30 years.
ts_offset = 978307200
...
def write_json_file(data, path):
    write_file(path, json.dumps(
        data, ensure_ascii=False, sort_keys=True, indent=1))
...
def paprika_markdownish(content, photos_dict):
    if content:
        content = re.sub(r'\[(photo):(.+?)\]', lambda x: '![' + x.group(2) +
                         '](/images/recipes/' + photos_dict[x.group(2)] + ')', content)
        ...
        content = re.sub(r'\[(recipe):(.+?)\]', lambda x: '[[' +
                         make_filename(x.group(2)) + '|' + x.group(2) + ']]', content)
        return content
    else:
...
def make_alpha_grouped_list(the_list, pops=None):
    ...
    for list_items in the_list:
        list_alpha_grouped.setdefault(
            list_items[0].lower(), []).append(list_items)
    if pops:
        for pop in pops:
...
# and copy the DB source dir over.
shutil.copytree(path_db_med, path_temp_working)
...
for e, category in enumerate(categories_dimensions):
    ...
    if not category['recipes']:
        ...
        cat_has_child = list(
            item for item in categories_dimensions if item['cat_parent_id'] == category['cat_id'])
...
for child in categories_dimensions:
    ...
    # create a copy of child, and add a "children" list
...
for id, child in children.items():
    ...
    if id != root_id:  # don't add the root to anything
        try:
            ...
            child['recipes_ids'] = dict(
                zip(child['recipes'], child['recipes_names']))
            child['recipes_nam'] = dict(
                sorted(zip(child['recipes_names'], child['recipes'])))
        except:
            pass
        try:
            del child['recipes']
...
recipes_sql = f"""
      SELECT
        ...
        (
          SELECT
            GROUP_CONCAT(DATE(M.ZDATE, 'unixepoch',
                         '+31 year', 'localtime'),"|")
            /*
...
for result in recipes_results:
    ...
    if result['categories']:
        try:
            result['categories'] = result['categories'].lower().split(
                '|')  # Forcing Cats to lowercase
            for cl in result['categories']:
                ...
                append_to_recipes = {'title': result['name'], 'permalink': result['permalink'], 'photo_thumb': result['photo_thumb'],
                                     'mine': result['mine'], 'rating': result['rating'], 'p_recipe_id': result['p_recipe_id']}
                ...
                tags[cl]['recipes'] = sorted(
                    tags[cl]['recipes'], key=itemgetter('title'))
                ...
                tags[cl]['rel_tags'] += [i for i in result['categories']
                                         if i not in remove_for_rel]

        except Exception as e:
            print("🛑 Something fubar in categories for " + recipe_filename + "\n")
            print(e)
            print("-------\n")
        result['categories'] = [i for i in result['categories']
                                if i not in cats_hack['remove']]
    ...
    def prep_ingredients(ing_str):
        ...
        ing_str = ing_str.replace('½', ' 1/2').replace('⅓', ' 1/3').replace(
            '¼', ' 1/4').replace('¾', ' 3/4').replace('⅔', ' 2/3').replace('⅛', '1/8')
    ...
    #         # removes everything after any *
    #         ing_2 = ing_2.split('*')[0]
    #         ing_2 = ing_2.strip()                   # removes left & right whitespace
    #         # ing_2 = re.sub('\[\^(\w+)\]','',ing_2)  # removes markdown footnotes
    #         # removes markdown footnotes
    #         ing_2 = re.sub(r'\[\^(\w+)\]', '', ing_2)
    #         # removes markodown _italic_
    #         ing_2 = re.sub(r'_(\w+)_', r'\1', ing_2)
    ...
    try:
        ...
        result['photos_dict'] = dict(
            zip(result['photos_names'], result['photos_filenames']))
    except:
    ...
    if result['photo_thumb'] or result['photos_dict']:
        ...
        if result['photo_thumb']:
            try:
                ...
                shutil.copy2(source_path_recipe_photos + "/" +
                             result['photo_thumb'], dest_path_recipe_photos + new_photo_thumb)
        ...
        if result['photo_large']:
            try:
                ...
                shutil.copy2(source_path_recipe_photos + "/" +
                             result['photo_large'], dest_path_recipe_photos + new_photo_large)
        ...
        if result['photos_dict']:
            ...
            for photo_name, pd2 in result['photos_dict'].items():
                ...
                try:
                    ...
                    shutil.copy2(source_path_recipe_photos + "/" + pd2,
                                 dest_path_recipe_photos + new_photo_filename)
                    ...
                    print("🛑 MISSING PHOTOS for " +
                          recipe_filename + ": " + new_photo_filename)
                    # print(e)
                    print("-------")
    ...
    if result['meal_dates'] and result['meal_types']:
        result['meal_notes'] = [x+'-'+y for x,
                                y in zip(result['meal_dates'], result['meal_types'])]
    ...
    if result['directions']:
        rdirections = paprika_markdownish(
            result['directions'], result['photos_dict'])
        rdirections = mdit.render(rdirections)
    else:
        rdirections = None

    if result['description']:
        rdescription = paprika_markdownish(
            result['description'], result['photos_dict'])
        rdescription = mdit.render(rdescription)
    else:
        rdescription = None

    if result['ingredients']:
        list_ing_lines = paprika_markdownish(
            result['ingredients'], result['photos_dict'])
    ...
    if result['notes']:
        result['notes'] = paprika_markdownish(
            result['notes'], result['photos_dict'])
    ...
    del (result2['html'], result2['directions'],
         result2['notes'], result2['nutritional_info'])
...
meals_indices['recipe_by_date_and_meal'] = defaultdict(
    dict)  # … RECIPE by DATE and MEAL
meals_indices['meal_by_recipe_and_date'] = defaultdict(
    dict)  # … MEAL by RECIPE and DATE
meals_indices['meal_by_date_and_recipe'] = defaultdict(
    dict)  # … MEAL by DATE and RECIPE
meals_indices['meal_data_by_recipe_and_status'] = defaultdict(
    dict)  # … MEAL GROUPED NOTES & DATA by RECIPE and STATUS
meals_indices['meal_data_by_status_and_date'] = defaultdict(
    dict)  # … MEAL GROUPED NOTES & DATA by STATUS and DATE
...
for data_meal in meals_results:
    ...
    meals_indices['recipe_by_date_and_meal'][meal_date][meal_type_name].append(
        append_meal_list)
    ...
    if os.path.isfile(note_file):
        ...
        try:
            append_featured = {
                'date': meal_date, 'type': meal_type_name, 'feature': parsed_note['feature']}
            try:
                meals_indices['meal_data_by_recipe_and_status'][meal_recipe_id]['featured']
            except:
                meals_indices['meal_data_by_recipe_and_status'][meal_recipe_id]['featured'] = [
                ]
            meals_indices['meal_data_by_recipe_and_status'][meal_recipe_id]['featured'].append(
                append_featured)
        except:
            try:
            ...
            meals_indices['meal_data_by_recipe_and_status'][meal_recipe_id]['regular'].append(
                append_nofile_regular)
    else:
        note_has_file = False
        try:
        ...
        meals_indices['meal_data_by_recipe_and_status'][meal_recipe_id]['nofile'].append(
            append_nofile_regular)
    ...
    if note_has_file:
        ...
        try:
            append_featured_data = {
                'recipe': meal_recipe_id, 'feature': parsed_note['feature']}
            try:
            ...
            meals_indices['meal_data_by_status_and_date']['featured'][meal_date][meal_type_name].append(
                append_featured_data)
            ...
            meals_indices['meal_data_by_status_and_date']['regular'][meal_date][meal_type_name].append(
                append_data)
        ...
        meals_indices['meal_data_by_status_and_date']['nofile'][meal_date][meal_type_name].append(
            append_data)
...
for meal_mkdn_file in sorted(os.listdir(path_meal_mkdn_files)):
    ...
    meal_mkdf_filename_pat = r"^(\d\d\d\d)-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[01])-(\w+).md$"
    ...
    if meal_mkdf_filename_mat:
        ...
        meal_uri_path = "/notes/" + meal_year + "-" + \
            meal_mnth + "-" + meal_day + "-" + meal_type + ".html"

        # Now, we need to open each file …
        if os.path.isfile(meal_mkdn_file_path):
            # See note "Parsed Note" at foot
            parsed_note = frontmatter.load(meal_mkdn_file_path)
            # and pull out the value of "feature" key.
            feature = parsed_note.get('feature')
            # If there is one, it is a "featured" meal note,
            if (feature):
                note_type = "feature"
                append_to_note = {'date': meal_date, 'type': meal_type,
                                  'feature': feature, 'uri_path': meal_uri_path}
            # If there isn't, it's a "rough" meal note
            else:
                note_type = "rough"
                append_to_note = {'date': meal_date,
                                  'type': meal_type, 'uri_path': meal_uri_path}
...
for page_mkdn_file in sorted(os.listdir(path_pags_mkdn_files)):
    ...
    if page_mkdf_filename_mat:
        ...
        if os.path.isfile(page_mkdn_file_path):
            # See note "Parsed Note" at foot
            parsed_page = frontmatter.load(page_mkdn_file_path)
...
for letter, tagos in tags_alpha_grouped.items():
    tags_alpha_grouped_2[letter] = {display_tag(tago): {'titlecase': display_tag(
        tago, 'titlecase'), 'filename': make_filename(tago)} for (tago) in tagos}
...
write_json_file(categories_dimensions, path_json_data +
                "categories_dimensions.json")
...
write_json_file(pages_alpha_grouped, path_json_data +
                "pages_alpha_grouped.json")
# ++++++++++++++++++++++++++++++++++++++
...
# "ignore errors" nukes it
shutil.rmtree(path_temp_working, ignore_errors=True)
print("\n✅ CLEANUP Complete\n")  # ++++++++++++++++++++++++++++++++++++++++++++
